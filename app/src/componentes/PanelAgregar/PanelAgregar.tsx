...
export function PanelAgregar({
  ...
  const elegirFiltro = (id: FiltroPrincipal) => {
    setFiltros((anteriores) => ({ ...anteriores, principal: id }));
  };

  const alternarCreditos = () => {
    setFiltros((anteriores) => ({
      ...anteriores,
      hastaSeisCreditos: !anteriores.hastaSeisCreditos,
    }));
  };
        ...
        <div className="panel-agregar__filtros" role="group" aria-label="Filtros">
          {FILTROS_PRINCIPALES.map((filtro) => (
            <Chip
              key={filtro.id}
              variante={
                filtros.principal === filtro.id ? "seleccionado" : "relleno"
              }
              onClick={() => {
          ...
          <Chip
            variante={filtros.hastaSeisCreditos ? "seleccionado" : "relleno"}
            onClick={alternarCreditos}
          >
            ≤ 6 cr
          </Chip>
        </div>
