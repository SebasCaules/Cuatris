...
describe("App", () => {
  ...
  it("sin historia, la ruta por defecto muestra el plan de estudios", async () => {
    ...
    // Rediseño: el progreso ya no acompaña al plan como columna derecha; vive
    // en el banner de la pestaña Cursada.
    expect(
      screen.queryByRole("region", { name: "Progreso" }),
    ).not.toBeInTheDocument();
  ...
  it("la pestaña Cursada queda marcada con #/cursada y muestra el carrusel", async () => {
    ...
    // El banner de progreso va arriba del carrusel (rediseño).
    expect(
      await screen.findByRole("region", { name: "Progreso" }),
    ).toBeInTheDocument();
  });

  it("con historia tiene las regiones de 13b; «Sugerir corrección» sigue inerte", async () => {
    sembrarPlanConHistoria();
    montar();
    await waitFor(() => {
      expect(
    ...
    // Rediseño: sin columna derecha; el progreso es un banner de Cursada.
  ...
  it("el banner de Cursada lista el título principal, las electivas y los minors", async () => {
    sembrarPlanConHistoria();
    window.location.hash = "#/cursada";
    montar();
    const banner = await screen.findByRole("region", { name: "Progreso" });
    // Rediseño: una fila compacta; los minors van por sigla y los títulos
    // secundarios viajan en el tooltip.
    for (const texto of [
      ...
      expect(banner).toHaveTextContent(texto);
    }
  });
