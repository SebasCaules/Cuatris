...
/**
 * Los dos filtros del panel (rediseño): **troncales** son las obligatorias
 * del plan (ciclos básico y profesional) y **electivas** el resto. Son
 * independientes: con los dos prendidos se ve todo; con los dos apagados, nada.
 */
export type Filtro = "troncales" | "electivas";

/** Texto de cada toggle, en el orden en que se dibujan. */
export const FILTROS: readonly { id: Filtro; texto: string }[] = [
  { id: "troncales", texto: "Troncales" },
  { id: "electivas", texto: "Electivas" },
];

export type Filtros = Record<Filtro, boolean>;

/** Lo que ve el panel al abrirse: todo. */
export const FILTROS_INICIALES: Filtros = {
  troncales: true,
  electivas: true,
};

/** Una materia es troncal si el plan la tiene como obligatoria. */
export function esTroncal(materia: Materia): boolean {
  return materia.ciclo !== "electiva";
}
...
export interface Oferta {
  ...
  horario: string;
  /** «Mié 10–12», la parte del horario sin aula. */
  franja: string;
  /** Las aulas del bloque, para dibujarlas grandes. */
...
export function ofertaDeFila(
  ...
  const franja =
    bloque === null
      ? ""
      : `${DIA_CORTO[bloque.dia]} ${etiquetaDeFranja(bloque.desde, bloque.hasta)}`;
  const horario =
    franja === ""
      ? ""
      : `${franja}${aulas.length === 0 ? "" : ` · ${aulas.join(" · ")}`}`;
 ...
 * cuatrimestre sugerido y después el código, con lo ya aprobado o en curso al
 * final: sigue en la lista —tachado y atenuado— pero no tapa lo que todavía
 * hay que planificar. Las materias que el plan marca como no vigentes nunca
 * aparecen: `buscar` las devuelve a propósito y filtrar por vigencia es
 * trabajo de este panel.
 *
 * Los dos toggles solo miran el ciclo: las bloqueadas siguen apareciendo con
 * su motivo, como dice el pie de 13c.
 */
export function filasDeResultados(opciones: OpcionesLista): Fila[] {
  ...
  for (const materia of ordenadas) {
    ...
    if (esTroncal(materia) ? !filtros.troncales : !filtros.electivas) {
      continue;
    }
