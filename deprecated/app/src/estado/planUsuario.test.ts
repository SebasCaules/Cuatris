...
describe("reducir", () => {
  ...
  // Doce colores desde DESIGN.md §5 (antes diez): el reciclado empieza en el
  // decimotercero.
  it("recicla la paleta después de doce materias", () => {
    ...
    expect(estado.colores["72.11"]).toBe(11);
    expect(estado.colores["72.12"]).toBe(0);
    expect(estado.colores["72.13"]).toBe(1);
  });
...
describe("migrar", () => {
  ...
  it("rechaza un color fuera de la paleta", () => {
    const roto = { ...planUsuarioInicial(), colores: { "93.18": 12 } };
