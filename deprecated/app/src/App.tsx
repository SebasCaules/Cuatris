 ...
 * «Agregar materia» de 13c, el modal «Elegir comisión» de 13d y la materia que
 * el panel está previsualizando en el carrusel).
 *
...
function Aplicacion({ datos }: { datos: DatosCargados }) {
  ...
  /** Materia bajo el puntero en el panel de agregar; el carrusel la previsualiza. */
  const [previsualizada, setPrevisualizada] = useState<Codigo | null>(null);
  ...
  const panelAgregar =
    panel !== null && ruta.vista === "cursada" ? (
      <PanelAgregar
        ...
        alPrevisualizar={setPrevisualizada}
      />
    ) : null;
